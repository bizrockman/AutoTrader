"""Smoke tests — does the Binance connector actually return real data?

These hit the real Binance API (public endpoints, no key needed).
...
async def test_get_price_returns_positive_float(connector):
    price = await connector.get_price("BTC/USDT")
...
async def test_get_ohlcv_returns_candles(connector):
    candles = await connector.get_ohlcv("BTC/USDT", "1m", limit=5)
...
async def test_get_orderbook_has_bids_and_asks(connector):
    ob = await connector.get_orderbook("BTC/USDT", limit=5)
...
async def test_get_recent_trades_returns_trades(connector):
    trades = await connector.get_recent_trades("BTC/USDT", limit=10)
...
async def test_get_funding_rate_returns_data_or_not_available(connector):
    fr = await connector.get_funding_rate("BTC/USDT")
...
async def test_get_open_interest_returns_data_or_not_available(connector):
    oi = await connector.get_open_interest("BTC/USDT")
    assert isinstance(oi, dict)
    # Spot symbol may not have OI — that's ok
